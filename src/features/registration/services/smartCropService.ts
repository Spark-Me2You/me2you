 ...
 *
 * NOTE: This service is now primarily used as a fallback.
 * Client-side face cropping via faceCropService is the primary approach.
 */
...
export const smartCropService = {
  ...
  /**
   * Alias for triggerSmartCrop (backward compatibility)
   */
  triggerCrop: async (imageId: string): Promise<void> => {
    return smartCropService.triggerSmartCrop(imageId);
  },
