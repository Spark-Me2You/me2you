...
export const discoveryService = {
  ...
  /**
   * Fetch a random public image from the organization
   *
   * This function:
   * 1. Queries images joined with users
   * 2. Filters by org_id and is_public = true
   * 3. Randomly selects one image
   * 4. Generates public URL for the image
   * 5. Returns image data with owner info
   *
   ...
   *
   * @example
   * const imageData = await discoveryService.fetchRandomImage('org-uuid-123');
   * if (imageData) {
   *   console.log(imageData.imageUrl); // Display URL
   *   console.log(imageData.owner.display_name); // "John Doe"
   *   console.log(imageData.owner.bio); // "Computer science student"
   * }
   */
  fetchRandomImage: async (orgId: string): Promise<RandomImageData | null> => {
    if (!orgId) {
