...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      return new Response(
        JSON.stringify({ success: false, error: 'Missing authorization header', error_code: 'MISSING_AUTH' }),
    ...
    const { data: authResult, error: authError } = await supabaseJWT.auth.getUser();
    if (authError || !authResult?.user) {
      console.error('[delete-user-images] JWT verification failed:', authError?.message);
      return new Response(
        JSON.stringify({ success: false, error: 'Invalid or expired token', error_code: 'INVALID_TOKEN' }),
        { status: 401, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    if (userError || !userRow) {
      console.error('[delete-user-images] User row not found:', userError?.message);
      return new Response(
        JSON.stringify({ success: false, error: 'User profile not found', error_code: 'USER_NOT_FOUND' }),
    ...
    if (!userRow.org_id) {
      ...
      return new Response(
        JSON.stringify({ success: false, error: 'Could not determine organization', error_code: 'ORG_NOT_RESOLVED' }),
    ...
    const [gestureResult, croppedResult] = await Promise.all([
      supabaseAdmin.from('gesture_image').select('id, storage_path').eq('owner_id', userId),
      supabaseAdmin.from('cropped_image').select('id, storage_path').eq('owner_id', userId),
    ]);

    if (gestureResult.error) {
      console.error('[delete-user-images] Failed to query gesture_image:', gestureResult.error.message);
      return new Response(
        JSON.stringify({ success: false, error: 'Failed to query image records', error_code: 'DB_ERROR' }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    if (croppedResult.error) {
      console.error('[delete-user-images] Failed to query cropped_image:', croppedResult.error.message);
      return new Response(
        JSON.stringify({ success: false, error: 'Failed to query image records', error_code: 'DB_ERROR' }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    ...
    const gesturePaths = (gestureResult.data ?? []).map((r) => r.storage_path).filter(Boolean);
    const croppedPaths = (croppedResult.data ?? []).map((r) => r.storage_path).filter(Boolean);
    // Dedupe in case any path appears in both tables
    const storagePaths = [...new Set([...gesturePaths, ...croppedPaths])];

    console.log(
      `[delete-user-images] Paths collected: gesture=${gesturePaths.length} cropped=${croppedPaths.length} unique=${storagePaths.length}`
    );

    // 5. Remove from storage (only the paths owned by gesture/cropped tables)
    let storageDeletedCount = 0;
    if (storagePaths.length > 0) {
      const { error: storageError } = await supabaseAdmin.storage.from('images').remove(storagePaths);
      if (storageError) {
        console.error('[delete-user-images] Storage removal failed:', storageError.message);
        return new Response(
          JSON.stringify({ success: false, error: 'Failed to delete image files', error_code: 'STORAGE_ERROR' }),
          { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
    ...
    if (gestureDeleteResult.error) {
      console.error('[delete-user-images] Failed to delete gesture_image rows:', gestureDeleteResult.error.message);
      return new Response(
        JSON.stringify({ success: false, error: 'Failed to delete image records', error_code: 'DB_ERROR' }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    if (croppedDeleteResult.error) {
      console.error('[delete-user-images] Failed to delete cropped_image rows:', croppedDeleteResult.error.message);
      return new Response(
        JSON.stringify({ success: false, error: 'Failed to delete image records', error_code: 'DB_ERROR' }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    ...
    return new Response(
      JSON.stringify({ success: false, error: error.message, error_code: 'SERVER_ERROR' }),
      { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );
  }
});
