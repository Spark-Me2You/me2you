...
serve(async (req) => {
  ...
  try {
    ...
    if (authError || !authResult?.user) {
      console.error('[delete-user-account] JWT verification failed:', authError?.message);
      return new Response(
    ...
    if (userError || !userRow) {
      console.error('[delete-user-account] User row not found:', userError?.message);
      return new Response(
    ...
    // 4. List all storage objects under {org_id}/{user_id}/
    ...
    if (listError) {
      console.error('[delete-user-account] Failed to list storage objects:', listError.message);
      return new Response(
        JSON.stringify({ success: false, error: 'Failed to list image files', error_code: 'STORAGE_ERROR' }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    const storagePaths = (storageFiles ?? []).map((f) => `${storagePrefix}/${f.name}`);
    console.log(`[delete-user-account] Found ${storagePaths.length} storage objects to remove`);

    // 5. Delete all storage objects (before auth user, so a retry is possible on failure)
    let storageDeletedCount = 0;
    if (storagePaths.length > 0) {
      const { error: storageRemoveError } = await supabaseAdmin.storage.from('images').remove(storagePaths);
      if (storageRemoveError) {
        console.error('[delete-user-account] Failed to remove storage objects:', storageRemoveError.message);
        return new Response(
    ...
    console.log(`[delete-user-account] Storage cleaned: ${storageDeletedCount} objects removed`);
